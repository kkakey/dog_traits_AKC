...
breed_rank_2020 <- cbind(breed_rank_2020, links)
write.csv(breed_rank_2020, "./data/breed_rank_2020_2020.csv", row.names=F)
```
...
for (num in seq_along(links)) {
  ...
  print(paste(num, breed_rank_2020[num,]$Breed))
  ...
  while (c <17) {
    ...
    c <- c + 1
    breed_table <- rbind(breed_table, c(breed_rank_2020[num,]$Breed, trait, 
  ...
  breed_img_table <- rbind(breed_img_table, c(breed_rank_2020[num,]$Breed , 
...
### save data
# write.csv(breed_traits, "./data/breed_traits.csv", row.names=F)
...
joined_data <- breed_traits %>%
  ...
  left_join(., breed_weight_image, on ="Breed")
...
breed_traits_wide <- pivot_wider(joined_data, 
  ...
  # join Image links
  left_join(., joined_data %>% select(Breed, Image), on="Breed") %>% distinct()

write.csv(breed_traits, "./data/breed_traits.csv", row.names=F)
saveRDS(breed_traits, "./data/breed_traits.rds")
```

```{r}
# information on each trait and score ranges
...
```{r}
write.csv(breed_rank_2020, "./data/breed_rank_2020.csv", row.names=F)
saveRDS(breed_rank_2020, "./data/breed_rank_2020.rds")
```
